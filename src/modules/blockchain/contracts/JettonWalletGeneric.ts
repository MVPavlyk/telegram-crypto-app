...
type JettonWalletGeneric_init_args = {
  $$type: 'JettonWalletGeneric_init_args';
  owner: Address;
  master: Address;
};

function initJettonWalletGeneric_init_args(src: JettonWalletGeneric_init_args) {
  return (builder: Builder) => {
    let b_0 = builder;
    b_0.storeAddress(src.owner);
    b_0.storeAddress(src.master);
...
export class JettonWalletGeneric implements Contract {
  static async init(owner: Address, master: Address) {
    return await JettonWalletGeneric_init(owner, master);
  }

  static async fromInit(owner: Address, master: Address) {
    const init = await JettonWalletGeneric_init(owner, master);
    const address = contractAddress(0, init);
    return new JettonWalletGeneric(address, init);
  }

  static fromAddress(address: Address) {
    return new JettonWalletGeneric(address);
  }

  readonly address: Address;
  readonly init?: { code: Cell; data: Cell };
  readonly abi: ContractABI = {
    errors: JettonWalletGeneric_errors,
  };
  ...
  async send(
    ...
    message: Transfer | InternalTransfer | Burn
  ) {
