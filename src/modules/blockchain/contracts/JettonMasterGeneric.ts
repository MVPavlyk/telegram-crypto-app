...
export function loadTransferNotification(slice: Slice) {
  ...
  let _forward_payload = sc_0.asCell();
  return {
    $$type: 'TransferNotification' as const,
    query_id: _query_id,
    amount: _amount,
    sender: _sender,
    forward_payload: _forward_payload,
...
export function loadBurnNotification(slice: Slice) {
  ...
  let _response_destination = sc_0.loadAddress();
  return {
    $$type: 'BurnNotification' as const,
    query_id: _query_id,
    amount: _amount,
    sender: _sender,
    response_destination: _response_destination,
...
type JettonMasterGeneric_init_args = {
  $$type: 'JettonMasterGeneric_init_args';
  owner: Address;
  jetton_content: Cell;
};

function initJettonMasterGeneric_init_args(src: JettonMasterGeneric_init_args) {
  return (builder: Builder) => {
    let b_0 = builder;
    b_0.storeAddress(src.owner);
    b_0.storeRef(src.jetton_content);
...
export class JettonMasterGeneric implements Contract {
  static async init(owner: Address, jetton_content: Cell) {
    return await JettonMasterGeneric_init(owner, jetton_content);
  }

  static async fromInit(owner: Address, jetton_content: Cell) {
    const init = await JettonMasterGeneric_init(owner, jetton_content);
    const address = contractAddress(0, init);
    return new JettonMasterGeneric(address, init);
  }

  static fromAddress(address: Address) {
    return new JettonMasterGeneric(address);
  }

  readonly address: Address;
  readonly init?: { code: Cell; data: Cell };
  readonly abi: ContractABI = {
    errors: JettonMasterGeneric_errors,
  };
  ...
  async send(
    ...
    message: ChangeContent | Transfer | ProvideWalletAddress | 'Stop Mint' | BurnNotification | ChangeOwner | Deploy
  ) {
